...
import { getMGAs, createMGA, updateMGA, deleteMGA } from "@/lib/mgas";
...
export async function GET(request: NextRequest) {
  ...
  const { data, error: fetchError } = await getMGAs(userId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error: createError } = await createMGA({ ...body, user_id: userId });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "MGA ID is required" }, { status: 400 });

    const { data, error: updateError } = await updateMGA(id, userId, updates);
...
export async function DELETE(request: NextRequest) {
  ...
  const { error: deleteError } = await deleteMGA(id, userId);
