...
async function resolveUserId(request: NextRequest, body?: any): Promise<{ userId: string; error: null } | { userId: null; error: string }> {
  ...
  if (error || !data?.id) {
    return { userId: null, error: "Unable to identify user." };
  }
...
export async function GET(request: NextRequest) {
  ...
  const { data, error: fetchError } = await getAgents(userId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error: createError } = await createAgent({
      ...body,
      user_id: userId,
    });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data, error: updateError } = await updateAgent(id, userId, updates);
...
export async function DELETE(request: NextRequest) {
  ...
  const { error: deleteError } = await deleteAgent(id, userId);
